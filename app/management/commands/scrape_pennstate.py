...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		base_url = 'http://www.gopsusports.com/sports/m-baskbl/mtt/psu-m-baskbl-mtt.html'
	...
	def scrape_team(self, home_url):
		...
		team = {'team': "Penn State Men's Basketball"}
		...
		for row in rows:
			...
			if len(cells) >= 8:
				...
				year = {'year': cells[5].get_text()}
				player_data.update(year)

				town = {'town': cells[-1].get_text()}
				player_data.update(town)
				school = {'school': cells[-2].get_text()}
				player_data.update(school)
