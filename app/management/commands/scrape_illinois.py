...
class Command(BaseCommand):
	...
	def handle(self, *args, **kwargs):
		base_url = 'http://www.fightingillini.com/sports/m-baskbl/m-baskbl-roster.html'
	...
	def scrape_team(self, home_url):
		...
		team = {'team': "Illinois Men's Basketball"}
		...
		for row in rows:
			...
			if len(cells) >= 8:
				...
				town = {'town': cells[-2].get_text()}
				player_data.update(town)
				school = {'school': cells[-1].get_text()}
